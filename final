...
BALL_RADIUS = 15  # Устанавливаем радиус шара
HOLE_RADIUS = 22  # Устанавливаем радиус лузы
BALL_SPEED = 10  # Устанавливаем максимальную скорость шара
FRICTION = 0.98  # Устанавливаем коэффициент трения для движения шара
WALL_FRICTION = 0.95  # Устанавливаем коэффициент трения для столкновения со стенами
COLLISION_FRICTION = 0.95  # Устанавливаем коэффициент трения для столкновения между шарами
STOP_THRESHOLD = 0.1  # Устанавливаем порог остановки шара
...
#Функция для определения новой позиции шара после фола
...
#Генерация текстуры дерева для внешней рамы
...
# Функция для отрисовки текстуры с закруглением
...
class Ball:
    ...
    #Инициализирует объект шара с заданными координатами, цветом и скоростью
    ...
    #  Обновляет позицию шара в зависимости от его скорости и коэффициента трения
    def move(self):
        if not self.in_hole:
            self.x += self.speed_x
            self.y += self.speed_y
    ...
    #Отрисовка шар на экране, если он не находится в лунке
    def draw(self):
        if not self.in_hole:
            pygame.draw.circle(screen, self.color, (int(self.x), int(self.y)), BALL_RADIUS)

    # Обрабатывает столкновение с другим шаром, вычисляя новые скорости и направления
    ...
    #Обрабатывает столкновения шара с границами стола, изменяя его скорость и позицию
    ...
    #Проверяет, попал ли шар в лужу, и обновляет его состояние при попадании
...
class Cue:
    ...
    # Инициализирует объект кия с параметрами для управления ударом
    def __init__(self):
    ...
    #Выполняет удар по шару, вычисляя силу и направление удара на основе позиции мыши
...
#Показывает правила игры и ждет нажатия клавиши или мыши для возврата в меню
def rules_screen():
...
# Основной игровой цикл, который обрабатывает события, обновляет состояние шаров и управляет ходами игроков
def game_loop():
    global cue_collision, selected_ball
